...
class TagCloud(object):
	...
	FONT_COLOR = ['#7e7e7e', '#989898', '#b8b8b8', '#d3d3d3', '#d2d2d2', '#e3e3e3', '#f0f0f0', '#f4f4f4', '#f9f9f9', '#ffffff']
	...
	# originals: FONT_SIZE = [15, 18, 20, 22, 24, 27, 30, 35, 40, 45]
	FONT_SIZE = [12, 14, 16, 18, 20, 22, 24, 26, 30, 32]
	...
	def draw(self, words, imageFilePath=None):
		...
		if imageFilePath is None:
			imageFilePath = str(uuid.uuid4()) + '.png'
	...
	def _checkOverlap(self, x, y, h, w):
		...
		for word in self.words_to_draw:
			#if overlaps
			if not ((x + w + buffer < word['x']) or
                    (word['x'] + buffer + word['w'] < x) or
                    (y + h + buffer < word['y']) or
                    (word['y'] + buffer + word['h'] < y)):
	...
	def _save(self):
		...
		self.image.save(self.imageFilePath, "PNG", quality=90)
...
if __name__ == '__main__':
	...
	words = [{"text": "coffee", "weight": 20296.0}, {"text": "love", "weight": 15320.0}, {"text": "day", "weight": 6860.0}, {"text": "like", "weight": 5521.0}, {"text": "follow", "weight": 5393.0}]
